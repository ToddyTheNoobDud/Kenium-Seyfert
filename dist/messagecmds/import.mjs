...
export const Command = {
  ...
  run: async (client, message) => { 
    const replyError = (msg) => message.reply({ content: msg, flags: 64 });
      ...
      try {
        ...
        for (let i = 0; i < urls.length; i += BATCH_SIZE) {
          ...
          const batchPromises = batch.map(url => {
            return client.aqua
              .resolve({ query: url, requester: message.member })
        ...
        await message.reply({ content: `Successfully imported ${tracks.length} song${plural}.`, flags: 64 });
